...
impl OriginProducer {
        ...
        /// Log all currently active broadcast paths.
        pub fn log_active_broadcasts(&self) {
    let state = self.state.lock();
    if state.active.is_empty() {
    } else {
        for path in state.active.keys() {
            tracing::info!(%path, "active broadcast");
        }
    }
}
...
pub fn publish<S: ToString>(&mut self, path: S, broadcast: BroadcastConsumer) {
    ...
    info!(%path, "Publishing new broadcast");

    if !self.state.lock().publish(path.clone(), broadcast.clone()) {
        warn!(%path, "Duplicate publish ignored (same broadcast)");
        return;
    }

    let state = self.state.clone().downgrade();
    debug!("State locked for broadcast cleanup task");

    web_async::spawn(async move {
        broadcast.closed().await;
        info!(%path, "Broadcast closed; removing from producer state");
...
pub fn publish_prefix(&mut self, prefix: &str, mut broadcasts: OriginConsumer) {
    ...
    if let Some(ref p) = prefix {
        info!(%p, "Starting to publish broadcasts with prefix");
    } else {
        info!("Starting to publish broadcasts with empty prefix");
    }
    ...
    web_async::spawn(async move {
        while let Some((suffix, broadcast)) = broadcasts.next().await {
            ...
            info!(%path, "Publishing broadcast from stream");
